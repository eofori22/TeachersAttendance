...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        ...
        if ($user_data && password_verify($current_password, $user_data['password'])) {
            ...
            $auth = new Auth($conn);
            if (method_exists($auth, 'validatePasswordStrength')) {
                $password_errors = $auth->validatePasswordStrength($new_password);
            } else {
                // Basic password validation if method doesn't exist
                $password_errors = [];
...
// Get teacher details
$stmt = $conn->prepare("SELECT username, full_name, email, role, qr_code, profile_image FROM users WHERE user_id = ?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();
            ...
            <div class="card shadow-sm h-100">
                <div class="card-body">
                    ...
                    <ul class="nav nav-pills flex-column gap-2">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="<?= $base_path ?>/teacher/profile.php">
                    ...
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <?php if (!empty($user) && isset($user['qr_code']) && $user['qr_code']): ?>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label text-muted">QR Code</label>
                                    <p class="mb-0">
                                        <a href="<?= $base_path ?>/teacher/qr_code.php" class="btn btn-sm btn-outline-primary">
